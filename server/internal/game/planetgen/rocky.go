...
func (ctx *planetGenContext) assignBasicBiomes() {
	...
	for i := 0; i < n; i++ {
		...
		if elevation < oceanLevel {
			tile.SurfaceType = game.BiomeSurfaceLiquid
		...
		if avgElevDiff >= MOUNTAIN_ELEVATION_DIFF_THRESHOLD {
			tile.SurfaceType = game.BiomeSurfaceSolid
			tile.Color = solidColor
			continue
		}

		tile.SurfaceType = game.BiomeSurfaceRegolith
	...
	for i := 0; i < n; i++ {
		...
		case game.BiomeSurfaceLiquid:
			...
			if !separated[material.StateSolid].IsEmpty() {
				tile.SurfaceType = game.BiomeSurfaceIce
		...
		case game.BiomeSurfaceSolid:
			...
			if !separated[material.StateSolid].IsEmpty() {
				tile.SurfaceType = game.BiomeSurfaceSnow
				tile.Color = separated[material.StateSolid].GetAverageColorForState(material.StateSolid).Reflective
			}
		}
	}
