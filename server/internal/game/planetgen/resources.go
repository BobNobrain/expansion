...
func (ctx *planetGenContext) generateResorces() {
	ctx.resources = make(map[int][]game.ResourceDeposit)

	resourcesToGenerate := make([]game.ResourceData, 0)
...
func (ctx *planetGenContext) pickRandomTileForResource() int {
	...
	if ctx.tiles[candidate2].SurfaceType == game.BiomeSurfaceSolid {
		return candidate2
	}
...
func (ctx *planetGenContext) placeResourceDeposit(res game.ResourceData) {
