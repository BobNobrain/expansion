...
func (ctx *unexploredSystemGenerator) generate() {
	...
	system.Stars = make([]game.Star, nStars)
	isSingular := nStars == 1

	for i := 0; i < nStars; i++ {
		data := generateStarData(ctx.rnd)
		system.Stars[i] = game.Star{
			Params: data,
		}
	}

	// we should sort the stars by their mass
	slices.SortFunc(system.Stars, func(a, b game.Star) int {
	...
	for i := 0; i < nStars; i++ {
		system.Stars[i].ID = game.CreateStarID(system.SystemID, i, isSingular)
	}
...
func generateStarData(rnd *rand.Rand) game.StarParams {
...
func (ctx *unexploredSystemGenerator) generateBinaryStarsOrbits() {
	...
	system.Orbits[mainStar.ID] = game.OrbitData{
		Center:  game.NoCelestialID,
		Ellipse: mainOrbit,
		// all angular params are zero
	}

	system.Orbits[secondaryStar.ID] = game.OrbitData{
		Center:   game.NoCelestialID,
...
func (ctx *unexploredSystemGenerator) generateTernaryStarsOrbits() {
	...
	ctx.system.Orbits[thirdStar.ID] = game.OrbitData{
		Center: game.NoCelestialID,
